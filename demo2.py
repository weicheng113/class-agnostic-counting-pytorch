...
def inference(args):
    ...
    pretrained_state_dict = torch.load("./car_adapt/checkpoints/model.0036.pth", map_location=torch.device('cpu'))
    model.load_state_dict(state_dict=pretrained_state_dict)
    for i in range(len(dataset)):
        t1 = timer()
        example = dataset[i]
        true_count = example["output_map"].sum()/100.
        ...
        pred_count = pred.sum()/100.
        error_rate = (pred_count - true_count).abs()/true_count
        if error_rate > 0.2:
            print(f"Pred count: {pred.sum()/100.:.2f}, True count: {true_count:.2f}")
            end_time = timer()
            ...
            f_name, f_ext = os.path.splitext(Path(image_path).name)
            heatmap_outpath = os.path.join("heatmap", f"{f_name}_heatmap2{f_ext}")
            heatmap.save(heatmap_outpath)
            print(f'Predicted heatmap visualization saved to {heatmap_outpath}')
